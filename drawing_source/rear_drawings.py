...
def enlarged_details(api,lower=False):
    ...
    r=2.5/2*s;c.setDash(2,2);c.circle(cx,cy,1.85*s,stroke=1,fill=0);c.setDash()
    c.circle(cx,cy,r,stroke=1,fill=0);c.setDash(4,3);c.line(cx-65,cy,cx+65,cy);c.line(cx,cy-55,cx,cy+55);c.setDash()
    c.line(cx+r*.707,cy+r*.707,950,630);c.setFont('Helvetica',12);c.drawString(955,630,'M3; tap drill DIA 2.500')
    c.setFont('Helvetica',10);c.drawString(955,614,'Dashed: extruded collar OD 3.700, '+('1.500 below strip' if lower else '1.300 below shelf'))
