...
with quantities for every configuration, a hardware list, a captive-nut
...
JOINS = [('Rail_square_nut_guide_strip_', 'Weld to the underside of its longitudinal mount rail with a 7.2 mm gauge between the two strips.'),
         ('Rail_welded_sheet_bridge_', 'Weld the bridge feet to the GPU tray after capturing the square nut.'),
         ('Motherboard_tray_welded_sheet_bridge_', 'Weld the bridge feet to the body floor after capturing the M3 nut.'),
         ...
         ('Lower_bank_retention_flange', 'Weld to the back face of the lower rear web after capturing the eight #6-32 nuts.'),
...
def signature(record):
    ...
    return (record['thickness_mm'], tuple(round(v, 2) for v in record['flat_size_mm']), round(record['flat_area_mm2'], 1), bends)
...
def fastener_spec(name, shape):
    ...
    if 'standoff' in name.lower(): return 'Harwin R30-1000802 M3 x 8 mm female-female hex spacer'
...
def main(build, out):
    ...
    items = {}; hardware = collections.defaultdict(collections.Counter); nuts = []; holds = collections.defaultdict(set)
    for config in CONFIGS:
        ...
        for p in parts:
            ...
            if spec and 'nut' in spec and 'captive' in p['name'].lower() and config in ('nine-u', 'modular'):
                near = min(sheets, key=lambda s: s['shape'].distance(p['shape']))
                c = p['shape'].Center()
                nuts.append(dict(configuration=CONFIGS[config], nut=p['name'], hardware=spec, captured_on=near['name'], x=round(c.x, 2), y=round(c.y, 2), z=round(c.z, 2)))
    rows = []
    for n, (key, item) in enumerate(sorted(items.items(), key=lambda kv: kv[1]['name']), 1):
        ...
        tapping = '21 x #6-32 UNC-2B through extruded collars; form the shelf before extruding' if item['name'].startswith('Full_width_twenty_one_slot_rear') else ''
        ...
        if tapping: scs_notes.append('extruded collars are not a standard option; tap the 1.22 mm sheet or use press-fit nuts')
    ...
    with (kit / 'captive_nut_schedule.csv').open('w', newline='') as f:
        w = csv.DictWriter(f, list(nuts[0])); w.writeheader(); w.writerows(nuts)
    for config in CONFIGS:
