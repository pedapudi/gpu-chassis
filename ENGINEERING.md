...
1. Form and deburr panels and emboss the bosses. Extrude and tap each formed thread after the adjacent bends are made. Press the lid locating studs into the body walls after finishing. Bolt each separate 3 mm rack ear to the body with M4 × 10 screws into the tapped body walls; the ears are not welded. Apply a finish compatible with the chosen hardware and mask the threads.
...
7. Install the perforated rear cover, then the lid. Pan-head M3 screws through the body walls thread into the cover's side returns: two per side on the 9U, one per side on the module. The lid slides on four flush-head press-in studs in the body walls, which engage L-slots in its side returns. Lower the lid 10 mm behind its seated position, slide it forward against the front carrier, then fit two M3 × 6 retention screws through its rear tabs into threads in the cover. To remove the lid, take out those two screws, slide it 10 mm rearward and lift it. To remove the cartridge, also remove the cover, disconnect the cartridge cables and remove the cartridge hold-down screws.
