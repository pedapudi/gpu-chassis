...
ap=argparse.ArgumentParser();ap.add_argument('--out',type=Path,required=True);ap.add_argument('--variant',choices=['both','nine-u','nine-u-180','nine-u-120','modular','modular-120'],default='both');a=ap.parse_args()
...
if a.variant in ('both','nine-u','nine-u-180','nine-u-120'):
 ...
 if a.variant!='nine-u':
  ...
  for name,mode in (('nine-u-180','2x180'),('nine-u-120','3x120')):
   ...
   selected,_=configure(parts,mode);export_variant(selected,json.loads((a.out/'nine-u/validation.json').read_text()),a.out/name,mode)
