...
7. Install the perforated rear cover, then the lid. All thumbscrews face rearward, so both are serviced from behind the rack and nothing projects beyond the body sides. The body side walls end in two inward rear flanges, each with two extruded M3 threads. Two M3 captive thumbscrews in the cover web thread into the lower flange threads. The lid reaches over the cover and folds two tabs down behind it; their captive thumbscrews pass through the cover and thread into the upper flange threads. At the front, L-slots in the lid side returns engage four flush-head press-in studs in the body walls. To remove the lid, loosen its two thumbscrews, slide it 10 mm rearward off the studs and lift it. Then loosen the two cover thumbscrews and withdraw the cover. To remove the cartridge, also disconnect its cables and remove the cartridge hold-down screws.
...
- Emboss the eight conical bosses (20 mm base, 10 mm top, 4.5 mm rise in 1.5 mm steel) in the body floor and GPU tray with a forming die. A fabricator without one can substitute M3 and M4 press-in standoffs of the same height.
- Accept or relocate holes closer than two thicknesses to a bend line. The EIA-310 rack holes sit 3.05 mm from the rack-ear bend because the rack standard fixes their positions; the M4 side-screw holes in the ear's side leg sit 5.7 mm from the same bend, against a 6 mm guideline. On the GPU rear panel, the outermost aperture sits 0.95 mm from the side-return bend; the backplane socket pitch fixes it, and the board leaves only 1 mm of side clearance for a shift. The shelf tap-drill holes sit 1.33 mm from the shelf bend, which is acceptable only because the collars are extruded after bending. `formed_part_checks.json` lists every case.
- Check the rear-corner clearances on the prototype. The outermost card brackets lift past the body rear-flange thread collars with 0.93 mm nominal clearance, and the GPU shelf corners are cut back to give the collars 2 mm as the cartridge lifts.
