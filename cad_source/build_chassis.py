...
P=dict(width=440.,depth=485.,height=354.8,sheet=1.5,gpu_deck_z=170.,rear_bracket_plane=469.,
       ...
       fan_mount_slot_length=9.,fan_mount_slot_width=4.8,vent_hole_diameter=8.,vent_pitch=10.,
...
def build(out,cache,fan_size=120,return_parts=False):
 ...
 def fan_slot(x,y,z,depth,length=9.):
  half=(length-4.8)/2
  return union([box(x-half,y,z-2.4,2*half,depth,4.8),cyl(x-half,y,z,2.4,depth,(0,1,0)),cyl(x+half,y,z,2.4,depth,(0,1,0))])
 ...
 vent_bottom=303.07;cover_side_z=(337,);lid_z=H-10;lid_studs=[(75,H-10),(380,H-10)]
 ...
 front=union([box(-21.3,F,0,482.6,2,H),box(1.5,F+2,2,1.5,18,H-4),box(437,F+2,2,1.5,18,H-4),box(1.5,F+2,2,18.5,1.5,H-4),box(420,F+2,2,18.5,1.5,H-4)])
 fc=[]
 for z in (85,Z+90):
  for x in (100,220,340):fc+=perforations(x,z,58,F-1,5)
  # Each boundary opening accepts the two screw axes 15 mm apart.
  for zz in (z-52.5,z+52.5):
   fc += [fan_slot(x,F-1,zz,5,9) for x in (47.5,392.5)]
   fc += [fan_slot(x,F-1,zz,5,24) for x in (160,280)]
 for x in (-12.55,452.55):
  for i in range(8):fc.append(cyl(x,F-1,21.825+44.45*i,3.5,5,(0,1,0)))
 fc += [side_holes(F+12,z) for z in panel_side_z]
 front=add('Front_panel_with_spot_welded_side_angles_and_rack_ears',front.cut(cq.Compound.makeCompound(fc)),'shell',dark);profile('front_face_no_returns',front,'y',F)
 ...
 # The upper rear cover is removed after the lid; side screws through the walls thread into its returns.
 vent,vent_folds,vh=rc.cover(W,D,vent_bottom,H-3,cover_side_z,lid_z)
 for row,z in enumerate((312,322,332,342)):
  for x in range(14+5*(row%2),427,10):
   if rc.perforation_allowed(W,H,x,z,lid_z):vh.append(cyl(x,D-2,z,4,4,(0,1,0)))
 ...
 for name,shape,group in rc.cover_fasteners(W,D,cover_side_z,lid_z):add(name,shape,group,gold if 'nut' in name else dark,visible=group!='lid_screws')
 # The lid top reaches over the cover and folds two rear tabs; its welded side returns carry L-slots for the wall studs.
 lid_top,lid_folds,lid_holes=rc.lid_top(W,D,H,F+2,lid_z)
 ...
 for x in (100,220,340):fan(f'Upper_120mm_fan_{x}',x,F+3.5,260,120,P['case_fan_depth'],'fans')
 ...
 partition_fix=[(8,35),(432,35),(8,122),(432,122)]
 partition=cut(box(1.5,F+2,168.5,437,123-F,1.5),passage_tools+[cyl(x,y,167,1.7,5) for x,y in partition_fix])
 add('Independently_removable_forward_cable_baffle',partition,'partition')
 profile('forward_cable_baffle_two_195x65_passages',partition,'z',168.5)
 for x,y in partition_fix:
  side=1.5 if x<220 else 426.5
  tab=box(side,y-6,167,12,12,1.5).cut(cyl(x,y,166,1.7,4))
  add(f'Baffle_side_welded_tab_{x}_{y}',tab,'guides')
  fast(f'Baffle_M3x6_{x}_{y}',(x,y,170),(0,0,-1),'M3',6,'partition_screws')
  add(f'Baffle_captive_nut_{x}_{y}',nut((x,y,167),(0,0,-1),'M3'),'guides',gold)
...
[(408.5,F+31.5,65),(422,50,70),(370,105,110),(270,145,95),(174,160,95),(174,185.5,75),(174,185.5,45.57)]]
 ...
 checks=dict(PSU_model='ASUS-PRO-WS-3000P',PSU_size_depth_width_height_mm=[175,150,86],PSU_rear_mount_holes_construction_xz_mm=atx,PSU_handedness='Rear-view counterclockwise quarter-turn of the standard ATX pattern',backplane_socket_count=12,backplane_GPU_socket_pitch_mm=40.64,backplane_auxiliary_end_slot_gap_mm=20.32,backplane_PCB_origin_construction_xy_mm=[BOARD_X,BOARD_Y],backplane_dimensions_mm=[429,225,2.5],backplane_mounting_holes_photo_estimates=True,motherboard_standoff_type="M3 x 8 mm male-female hex standoff, 6 mm stud",motherboard_stud_thread_engagement_mm=3.5,motherboard_upper_screw_engagement_mm=2.93,motherboard_stud_tip_to_floor_mm=0.5,backplane_supported_holes_construction_xy_mm=BOARD_MOUNT_POINTS,cartridge_removal_requires=['lid (two rear retention screws, then a 10 mm rearward slide)','upper rear vent and its fasteners','tray hold-down screws','disconnected harnesses'],enclosure_mm=[482.6,P['depth'],H],body_width_mm=W,rack_units=8,upper_rear_positions=P['upper_slot_count'],dual_slot_GPU_envelopes=10,single_width_auxiliary_card_envelopes=1,backplane_trailing_auxiliary_socket='Shares the last rear position with the tenth GPU second bracket; usable only without that GPU',lower_rear_positions=8,
  ...
  fan_mount_slot_mm=[9,4.8],shared_fan_mount_slot_mm=[24,4.8],fan_frame_pitch_mm=120,adjacent_fan_screw_gap_mm=15,vent_hole_diameter_mm=8,front_vent_pitch_mm=10,
  rear_upper_vent_bounds_xz_mm=[1.5,303.07,437,50.23],rear_upper_vent_hole_count=len(vh)-1,
  ...
  interdeck_passages_mm=[195,65],interdeck_passage_origins_xy_mm=[[15,55-F],[230,55-F]],interdeck_passage_corner_radius_mm=6,MCIO_edge_bank_centres_x_mm=[W-x for x in mcio_front_x],MCIO_additional_photo_locations_xy_mm=[[W-x,y-F] for x,y in photo_extra],MCIO_to_component_intersections=component_route_hits,external_MCIO_clear_aperture_mm=[139,25],MCIO_test_plug_cross_section_mm=[35,14],
 ...
 asm=cq.Assembly(name='Eight_unit_WRX90_GPU_chassis');cass=cq.Assembly(name='Removable_GPU_cassette');meshes=[]
 for a in parts:
  ...
  if a['role']=='fabricated' and a['group'] in ('shell','cassette','lid','rear_vent','guides','partition','strain_relief','mounts','motherboard_mounts','psu_support','fans') and not any(v in a['name'] for v in ('nut','screw','standoff','washer','M3','M4')):
